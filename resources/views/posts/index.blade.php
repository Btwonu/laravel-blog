	...
	<section class="section-form my-8">
		<div class="shell mx-auto max-w-screen-xl px-4">
			<div class="section__inner md:flex md:justify-between">
				<form method="get" action="{{ route('home') }}" class="form w-full justify-center md:flex md:flex-wrap md:justify-between">
					...
					<div class="form__field mt-4 flex justify-center md:mt-0 md:min-w-[20rem]">
						...
						<select id="s-category" class="select-info select w-full max-w-xs" name="category" value="{{ request('category') }}">
								...
								@foreach ($tags as $tag)
									<div class="tags__item">
										<div class="tag relative">
											...
											<span
												class="leading-sm inline-flex items-center rounded-full bg-orange-200 py-2 px-4 text-xs font-bold uppercase peer-checked:bg-orange-400">{{ $tag->title }}</span>
										</div><!-- /.tag -->
									</div><!-- /.tags__item -->
								@endforeach
							</div><!-- /.tags__items -->
